...
function Modify() {
  ...
  const fields = [["Add", "Edit", "Delete"], ["Artists", "Albums", "Streaming Services"]];
  ...
  const generateNames = () => {
    ...
    else if (active === 'Streaming Services')
          ...
          <Box display='flex' className={classes.teste} justifyContent='center' alignItems='center' >
            ...
            <Box display='flex' justifyContent='center' className={classes.teste} alignItems='center'>
              {create === 'Delete' ? (
                ...
                <Grid container>
                  {elementDecide().map((obj, index) => (
                    <Grid item key={index} lg={3} md={4} sm={6} xs={12} >
                      <Box display='flex' alignItems='flex-end'>
                        <Typography className={classes.titleBox} >{obj}</Typography>
